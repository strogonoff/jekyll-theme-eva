...
var burl=op.burl || ("http"+(/MSIE/i.test(navigator.userAgent)?"":"s")+"://outdatedbrowser.com/");
if (!op.url) {
    op.url= burl+((op.l && (op.l+"/"))||"")+""+(op.test?"":"");
}
function busprintf() {
    var args=arguments;
    var data = args[ 0 ];
...
var t={},ta;
t.en={msg:'Your web browser ({brow_name}) is out of date.',msgmore:'Update your browser for more security, speed and the best experience on this site.',bupdate:'Update browser',bignore:'Ignore',remind:'You will be reminded in a week.',bnever:'Never show again'}
//t.af='';
...
var style2='<style>.buorg {position:absolute;position:fixed;z-index:111111; width:100%; top:0px; left:0px; border-bottom:1px solid #A29330; text-align:center; cursor:pointer;   color:#000;     background-color: #fff8ea;    font: 18px Calibri,Helvetica,Arial,sans-serif;    box-shadow: 0 0 5px rgba(0,0,0,0.2);}'
    ...
    +'.buorg div a { color: #fff; text-decoration: none; box-shadow: 0 0 2px rgba(0,0,0,0.4); padding: 1px 10px; border-radius: 4px; font-weight: normal; background: #5ab400;    white-space: nowrap;    margin: 0 2px; display: inline-block;}'
...
if (!ta.msg && t.indexOf && t.indexOf("{brow_name}")===-1) {//legacy style
    ...
    div.innerHTML = '<div class="buorg-pad"><span class="buorg-icon"> </span>' + t + '</div>';
}
...
(document.getElementById("buorga")||document.getElementById("buorgig")).onclick = function(e) {
    ...
    if(!op.no_permanent_hide && ta.bnever && op.reminderClosed==24*7) {
        op.div.innerHTML='<div><span class="buorg-moremsg">'+ta.remind+'</span> <span class="buorg-buttons"><a id="buorgpermanent" href="javascript: void 0;" target="_blank">'+ta.bnever+'</a></span></div>' + style+style2;
        div.onclick=null;
